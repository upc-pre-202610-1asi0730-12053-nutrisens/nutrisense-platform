...
public interface IActivityLogCommandService
{
    ...
    /// <returns>The created <see cref="ActivityLog"/> on success, or a <see cref="LogManualActivityError"/> on failure.</returns>
    Task<Result<ActivityLog, LogManualActivityError>> Handle(LogManualActivityCommand command, CancellationToken ct = default);
    ...
    /// <param name="ct">Token to observe for cancellation.</param>
    /// <returns>True on success, or a <see cref="DeleteActivityLogError"/> on failure.</returns>
    Task<Result<bool, DeleteActivityLogError>> Handle(DeleteActivityLogCommand command, CancellationToken ct = default);
}
